...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        try:
            self.env_observer = env.observer
        except:
            print('not creating env.observer')
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            for idx, done in enumerate(dones):
                if done:
                    ep_reward = np.sum(self.ep_rewards)
                    r_sum = np.sum(self.ep_rewards)
                    r_len = len(self.ep_rewards)
                    wandb.log({"ep_len": r_len})
                    wandb.log({"ep_rewards": r_sum})
                    print('--------------------------')
                    print(f'episode had {r_len} steps')
                    try:
                        self.env_observer.log_episode_multimodal()
                    except:
                        pass 
                    if r_sum < -math.inf:
                        print(self.ep_rewards) # sumo sometimes close to inf values which should cause an error
                        raise
                    self.ep_rewards = list()
                    # Update stats
